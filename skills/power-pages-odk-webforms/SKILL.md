...
- On enhanced data model sites, Power Pages metadata is surfaced through `mspp_*` virtual tables such as `mspp_webrole`, `mspp_entitypermission`, and `mspp_websiteaccess`; direct Dataverse reads against the old `adx_*` logical tables can fail with metadata-cache errors even when PAC package files still use `adx_*` YAML names. Diagnose target metadata with `mspp_*` reads.
...
- When `pac pages upload --modelVersion Enhanced` reports `XRM Network error: Entity adx_entitypermission ... Expected non-empty Guid`, first verify the target with `mspp_entitypermission` and `mspp_webrole`, not `adx_entitypermission` / `adx_webrole`. If the referenced web role exists but the table-permission row does not, treat the upload as failing to materialize Power Pages security metadata; create or save the missing Global table permission in Power Pages Security workspace or Power Pages Management app, then sync/cache-clear and verify through the browser `/_api`.
...
- Treating enhanced-model Power Pages security metadata as ordinary `adx_*` Dataverse tables. Microsoft documents enhanced data model metadata as `mspp_*` virtual tables backed by `powerpagecomponent`; use `mspp_*` for read-back diagnostics and the Security workspace/Power Pages Management app for fragile table-permission creation when PAC upload cannot resolve relationship GUIDs.
