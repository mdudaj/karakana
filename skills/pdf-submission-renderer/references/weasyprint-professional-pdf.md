...
- Omit running headers for clean client packs unless the user specifically requests them. Avoid header rules when they look inconsistent across pages.
...
- Use `@page` margin boxes for page counters. Add running headers only when the design calls for them.
...
- Do not add a running header for the current client pack.
...
- Review the first page for:
  - no running header unless requested;
