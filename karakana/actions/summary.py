...
- Escalate routine coding from `gpt-6-luna` to `gpt-6-sol` for consequential complexity, risk, or two unsuccessful diagnostic attempts; not every failing test.
- Escalate to `gpt-6-sol` only for authentication, payments, migrations, process state, production deployment risk, high-risk review, or repeated failures.
- If Sol remains stuck, stop and replan; Astra requires explicit exceptional-task selection.
