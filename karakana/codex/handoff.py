...
class CodexHandoffBuilder:
    ...
    def build_from_action_bundle(
        ...
        for action in actions:
            task = _task_from_action(bundle, action, project=project, skill=skill)
...
def _task_from_action(bundle: ActionBundle, action: ExtractedAction, project: str | None, skill: str | None) -> CodexHandoffTask:
    ...
    return CodexHandoffTask(
        ...
        context=_context_for_action(bundle, action),
        safety_rules=[
            "Do not commit.",
            "Do not push.",
        ...
        metadata={"artifacts": [action.metadata.get("artifact_path")] if action.metadata.get("artifact_path") else []},
    )
...
def _context_for_action(bundle: ActionBundle, action: ExtractedAction) -> str:
